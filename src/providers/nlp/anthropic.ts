...
export class AnthropicProvider implements NLPProvider {
  ...
  async moderateText(text: string): Promise<ProviderResult> {
    ...
    try {
      ...
      const moderationResult = this.parseModerationResponse(content.text);
...
Response (JSON only):`;
  ...
  private parseModerationResponse(
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        for (const [category, v] of Object.entries(parsed as Record<string, unknown>)) {
          ...
          if (!Number.isNaN(confidence)) {
            const mappedCategory = mapProviderCategory(category, 'anthropic');
      ...
      console.warn(
        'Failed to parse Anthropic moderation response:',
